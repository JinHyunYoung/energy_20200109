...
<%
	...
	*********************************************************************************************************************************************/
	...
	String sSiteCode				= "F496";			// IPIN 서비스 사이트 코드		(NICE평가정보에서 발급한 사이트코드)
	String sSitePw					= "95234317";			// IPIN 서비스 사이트 패스워드	(NICE평가정보에서 발급한 사이트패스워드)
	...
	*/
	...
	String sReturnURL				= "http://"+getDomain+"/web/common/ipin_result.do";
...
function ipinPopup(){
	//var child = window.open("https://cert.vno.co.kr/ipin.cb", 'form_ipin', 'fullscreen=no, menubar=no, status=no, toolbar=no, titlebar=yes, location=no, scrollbar=no');
	
	//document.form_ipin.target = "form_ipin";
...
<form name="form_ipin" method="post" action="">
	...
	<input type="hidden" name="param_r1" value="<%=request.getParameter("no")%>">
	<input type="hidden" name="param_r2" value="<%=request.getParameter("param2")%>">
	<input type="hidden" name="param_r3" value="<%=request.getParameter("param3")%>">
