...
<%
	 	...
	 	if (iRtn == 1) {
	    	session.setAttribute("AuthVirtualNo"	, sVNumber);
	    	session.setAttribute("AuthRealName" 	, sName);
	    	session.setAttribute("AuthBirthDate"	, sBirthDate);
	    	session.setAttribute("AuthMobileNo" 	, "");
	    	session.setAttribute("AuthDupInfo"  	, sDupInfo);
	    	session.setAttribute("AuthGender"  		, sGenderCode);
	    	session.setAttribute("AuthType"  		, "ipin");
	    	session.setAttribute("Authfirst"  		, "1");
  //  	if(sReserved1.equals("00")){			// 회원가입
%>
     	<script type="text/javascript">
	    	...
	    	data['num']			=	"";
			data['authtype']	=	"2";
			data['di']			=	"<%=sDupInfo%>"; <%-- data['ci']			=	"<%=sCoInfo1%>"; --%>
			
			data['gender']		=	"<%=sGenderCode%>";
