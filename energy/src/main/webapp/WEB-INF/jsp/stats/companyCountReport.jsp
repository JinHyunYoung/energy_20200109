...
<script type="text/javascript">
	...
	var indsTypeUrl = "<c:url value='/web/stats/companyCountReport.do'/>";
	// 조직 리스트 불러오기
	function statistics(){
		$.ajax(
				{
			    	...
			    	success: function( data )
			      	{
		        		if( data != null &&data.industry != null ){
		            		drawChart("chart_container1", data.industry,"업종", "#f68b12");
		        		}else{
		    	          	alert("업종별 데이타가 없습니다.");
		        		}
		        			
		        		if( data != null &&data.scale != null ){
		            		drawChart("chart_container2", data.scale,"사업장규모", "#42aeb8");
		        		}else{
		    	          	alert("규모별 데이타가 없습니다.");
		        		}
...
<!-- stati_search_area -->
		<div class="stati_search_area">
			<dl class="srh_dl">
				...
				<dd class="srh_dd">
					<select class="in_wp130" id="bs_yy" name="bs_yy" onchange="javascript:frmSubmit()">
		...
		<div class="division">
			...
			<div class="area48 float_left">
				<div class="graph_box">
					...
					<p class="graph_txt2">&lt; 사업체 업종별 사업체 수 &gt;</p>
				</div>
				<div class="graph_infobox">
					<c:forEach var="item" items="${infoList}">
						<c:if test="${item.stats_info_m_cd eq '01-01'}">${item.contents}</c:if>
					</c:forEach>
				</div>
			</div>
			<!--// left_area -->
			...
			<div class="area48 float_right">
				<div class="graph_box">
					...
					<p class="graph_txt2">&lt; 사업체 규모별 사업체 수 &gt;</p>
				</div>
				<div class="graph_infobox">
					<c:forEach var="item" items="${infoList}">
						<c:if test="${item.stats_info_m_cd eq '01-02'}">${item.contents}
						</c:if>
					</c:forEach>
				</div>
			</div>
		...
		<div class="division">
			...
			<div class="area48 float_left">
				<div class="graph_box">
					...
					<p class="graph_txt2">&lt; 사업체 업종별 사업체 수  추이&gt;</p>
				</div>
				<div class="graph_infobox">
					<c:forEach var="item" items="${infoList}">
						<c:if test="${item.stats_info_m_cd eq '01-03'}">${item.contents}
						</c:if>
					</c:forEach>
				</div>
			</div>
			...
			<div class="area48 float_right">
				<div class="graph_box">
					...
					<p class="graph_txt2">&lt; 사업체 규모별 사업체 수  추이&gt;</p>
				</div>
				<div class="graph_infobox">
					<c:forEach var="item" items="${infoList}">
						<c:if test="${item.stats_info_m_cd eq '01-04'}">${item.contents}
						</c:if>
					</c:forEach>
				</div>
			</div>
	...
	<h5 class="title">사업체 주요지표</h5>
		<!-- table_area -->
		<div class="table_area">
			<table class="statistics_list">
				<caption>사업체 주요지표 화면</caption>
				<colgroup>
					<col style="width: *;" />
				...
				<tr>
					...
					<th scope="col" colspan="2">2015년</th>
					<th scope="col" colspan="2">2016년</th>
					<th scope="col" colspan="2" class="last">2015~2016년 비교</th>
				</tr>
				<tr>
					<th scope="col" class="first">전체</th>
					<th scope="col">업체당</th>
					<th scope="col">전체</th>
					<th scope="col">업체당</th>
					<th scope="col">전체</th>
					<th scope="col">업체당</th>
					<th scope="col">전체</th>
					<th scope="col" class="last">업체당</th>
				</tr>
				</thead>
				<tbody>
				<c:forEach var="indices" items="${indicesList}">
				
				<tr>
					<td class="first bold">${indices.nm}</td>
					<td><span class="amount">${indices.point_3}</span></td>
					<td><span class="amount">
						<c:choose>
							<c:when test="${indices.stats_s_cd eq '01-01-01'}">-</c:when>
							<c:otherwise>${indices.cnt_3}</c:otherwise>
						</c:choose>
						</span>
					</td>
					<td><span class="amount">${indices.point_2}</span></td>
					<td><span class="amount">
						<c:choose>
							<c:when test="${indices.stats_s_cd eq '01-01-01'}">-</c:when>
							<c:otherwise>${indices.cnt_2}</c:otherwise>
						</c:choose>
						</span>
					</td>
					<td><span class="amount">${indices.point_1}</span></td>
					<td><span class="amount">
						<c:choose>
							<c:when test="${indices.stats_s_cd eq '01-01-01'}">-</c:when>
							<c:otherwise>${indices.cnt_1}</c:otherwise>
						</c:choose>
						</span>
					</td>
					<td>
					<span class="amount">${indices.point_4}</span>
					 (${indices.point_5}%)</td>
					<td class="last"><span class="amount">
						<c:choose>
							<c:when test="${indices.stats_s_cd eq '01-01-01'}">-</c:when>
							<c:otherwise>${indices.cnt_4}</c:otherwise>
						</c:choose>
						</span>
					</td>
				</tr>
		...
		<div class="graph_infobox">
			<c:forEach var="item" items="${infoList}">
				<c:if test="${item.stats_info_m_cd eq '01-05'}">${item.contents}
				</c:if>
			</c:forEach>
		</div>
		<!--// graph_infobox -->
