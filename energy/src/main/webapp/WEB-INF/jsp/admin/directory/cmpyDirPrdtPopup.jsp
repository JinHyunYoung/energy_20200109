...
function insertCmpyDirPrdtRow(){
	...
	if ( $("#cmpyDirPrdtFormRow").parsley().validate() ){
		...
		str += '<tr>';
		str += '	<td id="wbiz_tp_l_cd_label">' + wbiz_tp_l_cd_nm + '</td>';
		str += '	<td id="wbiz_tp_m_cd_label">' + wbiz_tp_m_cd_nm + '</td>';
		str += '	<td id="wbiz_tp_s_cd_label">' + wbiz_tp_s_cd_nm + '</td>';
...
function deleteCmpyDirPrdtRow(){
	...
	if(selectedRow != null && selectedRow.attr("id") == null){
		...
		if($('#cmpyDirPrdtTable > tbody').children().length == 0 ){
			...
			str += '<tr id="emptyRow">';
			str += '	<td colspan="4">등록된 주요제품 및 기술현황이 없습니다.</td>';
			str += '</tr>';
				...
				<div class="division">				
					...
					<form id="cmpyDirPrdtForm" name="cmpyDirPrdtForm" method="post" onsubmit="return false;">
						...
						<div class="title_area">
							<h2 class="pop_title">주요제품 및 기술현황</h4>
						</div>
						<!--// title_area -->
						
						<!-- table_area -->
						<div class="table_area">
							<table id="cmpyDirPrdtTable" class="list" style="min-width: 500px">
								...
								<tbody>
									<c:forEach items="${cmpyDirPrdtList}" var="list">	
										<tr>
											<td id="wbiz_tp_l_cd_label">${list.wbiz_tp_l_cd_nm}</td>
											<td id="wbiz_tp_m_cd_label">${list.wbiz_tp_m_cd_nm}</td>							
											<td id="wbiz_tp_s_cd_label">${list.wbiz_tp_s_cd_nm}</td>
											<td id="prdt_nm_label"><a title="주요제품" href="#" id="selectRow" name="selectRow">${list.prdt_nm}</a></td>
					...
					<!--// button_area -->
						...
						<div id="dataRowTitleDiv" class="title_area" style="display: none">
							<h2 class="pop_title">주요제품 및 기술현황 등록/수정</h4>
						</div>
						<!--// title_area -->
						...
						<div id="dataRowTableDiv" class="table_area" style="display: none">
							...
							<table class="write">
								...
								<tbody>
									<tr>
										<th scope="row">
											물산업분류<strong class="color_pointr">*</strong>
										</th>
										<td>
											...
											<label for="inds_tp" class="hidden">구분선택</label>
											
											<select class="in_wp120" id="wbiz_tp_l" name="wbiz_tp_l" data-parsley-errors-container="#errMsgManagerwbiz" data-parsley-required="true">
												<option value="">1Depth</option>
											</select>
											<label for="wbiz_tp_l" class="hidden">1Depth 선텍</label>
											
											<select class="in_wp120" id="wbiz_tp_m" name="wbiz_tp_m" data-parsley-errors-container="#errMsgManagerwbiz" data-parsley-required="true">
												<option value="">2Depth</option>
											</select>
											<label for="wbiz_tp_m" class="hidden">2Depth 선텍</label>
											
											<select class="in_wp120" id="wbiz_tp_s" name="wbiz_tp_s" data-parsley-errors-container="#errMsgManagerwbiz" data-parsley-required="true">
												<option value="">3Depth</option>
											</select>
											<label for="wbiz_tp_s" class="hidden">3Depth 선텍</label>
											<div class="parsely-single-error" id="errMsgManagerwbiz" />
										</td>
									</tr>
									<tr>
										<th scope="row">
											<label for="nm">세부 제품명(국문)<strong class="color_pointr">*</strong></label>
										</th>
										<td>
									...
									<tr>
										<th scope="row">
											<label for="nm_en">세부 제품명(영문)<strong class="color_pointr">*</strong></label>
										</th>
										<td>
