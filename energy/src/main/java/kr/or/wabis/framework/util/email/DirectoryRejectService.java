...
public class DirectoryRejectService extends EmailService {

	public DirectoryRejectService() {
	}
	...
	public long sendEmail(String receive_email, Map<String, Object> param) throws Exception {
		...
		try {			
			...
			String title = "기업디렉토리 신청이 반려되었습니다.";
	...
	private void makeEmail(Map<String, Object> param) throws Exception {
		...
		String replyHtml = IOUtils.toString(getClass().getResourceAsStream("/email/directoryReject.html"), "UTF-8");
		...
		replyHtml = replyHtml.replaceAll("#app_dt#", StringUtil.nvl(param.get("app_dt")));
		replyHtml = replyHtml.replaceAll("#manager_nm#", StringUtil.nvl(param.get("manager_nm")));
		
		
		replyHtml = replyHtml.replaceAll("#rjt_rsn#", StringUtil.nvl(param.get("rjt_rsn")));
