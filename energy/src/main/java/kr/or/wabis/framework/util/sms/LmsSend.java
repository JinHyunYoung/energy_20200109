 ...
 * <p>
 * Description: 
 * 서울시 SMS전송 시스템의 외부연동은 HTTP로 개발되어 있습니다. 
 * HTTP호출을 통해 메세지를 전송할 수 없는 경우나 개발상 편의를 위해 이 Helper클래스를 사용할 수 있습니다. 
 * 배포된 페키지에는 SmsSend.class 와 SmsSend.properties가 포함이 되어 있습니다. 
 * SmsSend.properties 는 서울시 SMS전송 시스템과의 연동에 필요한 설정을 가지고 있습니다. <br>
 * 이 라이브러리를 사용하기 위해서는 우선 서울시 SMS전송 시스템 담당자로 부터 아이디를 발급받아야 합니다. 
 * 이 인증 정보는 SmsSend.properties에서 Default.ID 와 Default.PW를 통해 설정을 할 수 있으며 setId , setPw 메소드를 통해서도 설정할 수 있습니다.
 * </p>
 * <p>
...
public class LmsSend implements SmsSendInterface {
    ...
    static {
        ...
        try {
            ...
            try {
                ...
                defaultPage = (String) ProjectConfigUtil.getProperty("sms.lms_page");
    ...
    /**
     * 인스턴스 생성 후 마지막 발생한 Exception 반환. 대부분 에러코드가 COMM_ERROR(-30) 일 때 lastException을 사용할 수 있음. 발생한 Exception이 없다면 null을 반환.
     * 
     ...
     */
    public LmsSend(String receiver, String subject, String msg, String sender, String reserve) {
     ...
     * @return 성공시 전송요청 일련번호 (양수), 오류 시 -10,-20,-30(음수)
     */
    public long sendMessage(String receiver, String subject, String msg, String sender, String reserve) {
        setReceiver(receiver);
        setSubject(subject);
        setMsg(msg);
        setSender(sender);
    ...
    private static long executeLmsSendHttp(LmsSend lms) {
        ...
        try {
            ...
            request.append("?id=").append(URLEncoder.encode(lms.id != null ? lms.id : ""));
            request.append("&pw=").append(URLEncoder.encode(lms.pw != null ? lms.pw : ""));
            request.append("&receiver=").append(lms.receiver != null ? lms.receiver : "");
            request.append("&receivername=").append(URLEncoder.encode(lms.receiverName == null ? "" : lms.receiverName));
            ...
            request.append("Host:").append(lms.host + (lms.port != 80 ? ":" + lms.port : "")).append("\r\n");
            request.append("\r\n");
            ...
            lms.lastException = (Exception) thr0;
            log(thr0);
