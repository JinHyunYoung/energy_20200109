 ...
 *
 * <p>
 * This class works like {@link StringTokenizer}; each time {@link #parseNext()} is invoked, the parser "moves" to the next key/value pair, which you can then obtain with {@link #getKey()} and {@link #getValue()}. The {@link #hasNext()} method shall be used to check if there's a next key/value pair
    ...
    public void parseNext() {
        ...
        if (isAt('\'') || isAt('"')) {
            ...
            while (true) {
                if (isAt(quote)) {
                    value = buf.toString();
                    // consume endquote and ','. this may put as beyond EOL but
                    // that's fine.
                    idx += 2;
                    return;
                }
